...
function classifyPath(
  ...
  if (isSymbolicLink) {
    if (!context.options.followSymlinks) return 'skip'
  ...
  if (context.options.exclude?.(rel)) return 'skip'

  if (directory) {
    const segments = rel.split('/')
    const name = segments[segments.length - 1]
    if (name && DEFAULT_IGNORED_DIRECTORIES.has(name)) return 'skip'
    return 'directory'
  }
...
function setEntry(context: ScanContext, key: string, filePath: string): void {
  if (
    Object.prototype.hasOwnProperty.call(context.entries, key) &&
    context.entries[key] !== filePath &&
    context.options.onCollision !== 'overwrite'
  ) {
    throw new Error(
      `Duplicate entry key "${key}" for ${context.entries[key]} and ${filePath}`
    )
  }
...
function scanDir(
  ...
  if (context.visitedDirectories.has(realDirectory)) return
