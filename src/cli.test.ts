...
function fixture(exports: Record<string, unknown> = {}): string {
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'fixture', exports }, null, 2) + '\n')
  return root
}
...
describe('runCli', () => {
  ...
  it('preserves custom exports when generating', () => {
    ...
    expect(JSON.parse(readFileSync(join(project, 'package.json'), 'utf-8')).exports).toEqual({
      './custom': './custom.js',
      '.': {
